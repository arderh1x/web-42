import { fetchUsers, getNewUsers, getWeather, getGallery } from './api.js'
...
function init() {
    ...
    /*labwork5 - d) - add gallery */
    const galleryBtn = document.createElement("button");
    galleryBtn.textContent = "Gallery";

    const buttons = [userRating, news, contacts, about, galleryBtn];
    buttons.forEach(btn => btn.addEventListener("click", onHeaderBtnClick));
    galleryBtn.addEventListener("click", onGalleryClick);
    header.append(...buttons);


    /* 5) */
    const currentUsersEl = document.createElement("div");
    currentUsersEl.innerHTML = "Current users: <b id='count'>0</b>";

    const newUsersEl = document.createElement("div");
    newUsersEl.innerHTML = "New users: <b id='newUsers'>Loading...</b>";

    footer.append(currentUsersEl, newUsersEl);

    getNewUsers().then(users => {
        const newUsers = users.map(user => user.firstname + " " + user.lastname).join(", ");
        newUsersEl.querySelector("#newUsers").textContent = newUsers;
    });
    ...
    setTimeout(() => {
        ...
        // labwork5 - added div-container for proper gallery work
        const usersContainer = document.createElement("div");
        usersContainer.id = "usersContainer";
        usersContainer.append(pContent, getUsers);
...
function onHeaderBtnClick() { // idk why I kept listener in init  
    const content = document.querySelector("#content");
    if (content.querySelector("h1")) content.querySelector("h1").remove();
    if (content.querySelector("#gallery")) content.querySelector("#gallery").remove();
    if (content.querySelector("#usersContainer")) content.querySelector("#usersContainer").style.display = "block";

    const h1Content = document.createElement("h1");
    h1Content.textContent = this.textContent;
...
async function onGalleryClick() {
    const content = document.querySelector("#content");
    if (content.querySelector("#usersContainer")) content.querySelector("#usersContainer").style.display = "none";
    ...
    gallery.forEach(imagePath => {
        const imgSrc = `/gallery/${imagePath}`;
        const image = document.createElement("img");
        image.src = imgSrc;

        image.addEventListener("click", () => window.location.href = imgSrc); // idk why not
        galleryEl.append(image);
    });

    content.append(galleryEl);
}
...
function setLeftPanel() {
    setTimeout(async () => {
        ...
        city.innerHTML = "City: <b id='city'>Loading...</b>";
        const temperature = document.createElement("p");
        temperature.innerHTML = "Temperature: <b id='temperature'>Loading...</b>";

        weatherBlock.append(city, temperature)
        document.querySelector("#leftPanel").append(searchInput, searchBtn, weatherBlock);
    ...
    setInterval(async () => {
        ...
        document.querySelector("#city").textContent = weather.city;
        document.querySelector("#temperature").textContent = weather.temperature;
    }, 5000); // put 60000 - 1 minute, 5000 - for testing
}
